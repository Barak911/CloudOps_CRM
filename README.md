...
Most recent live bootstrap — clean account → custom VPC → working stack, on the architecture at HEAD:
[verify + integration-test run #28773591366](https://github.com/Barak911/CloudOps_CRM/actions/runs/28773591366)
(all four jobs green). Raw captures in [`docs/proof/`](docs/proof/).

```text
$ kubectl get nodes -o wide          # 10.0.x = private subnets, custom VPC
ip-10-0-26-207.ec2.internal  Ready  10.0.26.207  v1.34.9-eks-7d6f6ec
ip-10-0-40-166.ec2.internal  Ready  10.0.40.166  v1.34.9-eks-7d6f6ec
ip-10-0-5-255.ec2.internal   Ready  10.0.5.255   v1.34.9-eks-7d6f6ec

$ kubectl get applications -n argocd
crm-manifests / crm-stack / prometheus-stack / root — all Synced + Healthy (ArgoCD v3.4.3)

$ kubectl get pdb -n crm             # incl. MongoDB quorum protection, 1 allowed disruption
$ kubectl get networkpolicies -A | wc -l   # 14, enforced by VPC CNI
# Karpenter 1.12.1: Pending pod → spot node provisioned in ~48s (private subnet, node shared SG)
# All 8 SLO/platform alerts loaded and inactive on a healthy cluster
```

> The bootstrap of this exact configuration surfaced eight real bugs (a
> plan-time `for_each` failure, a subnet-tag ownership war, an ESO webhook
> race, a PDB selector matching hook Jobs, a helm same-version-repackage
> cache trap, a Karpenter security-group mismatch, a TTL race in the verify
> workflow, and stale version pins) — each is fixed in code with the failure
> mode documented at the fix site. That story is the point of this repo.

## Architecture (diagrams-as-code)

Generated by [`docs/diagrams/generate.py`](docs/diagrams/generate.py) — regenerate after architecture changes.

**AWS infrastructure** — custom VPC, private nodes, Karpenter burst capacity:
![AWS infrastructure](docs/diagrams/01-aws-infrastructure.png)

**Kubernetes topology** — what ArgoCD reconciles:
![Kubernetes topology](docs/diagrams/02-kubernetes-topology.png)

**Request & telemetry flow:**
![Request flow](docs/diagrams/03-request-flow.png)

**GitOps delivery** — CI has zero cluster access:
![GitOps delivery](docs/diagrams/04-gitops-delivery.png)

## See it running

**ArgoCD — all applications Healthy/Synced** (app-of-apps, v3.4.3):
![ArgoCD applications](docs/screenshots/argocd-app-of-apps.png)

**ArgoCD — crm-stack resource tree** (MongoDB StatefulSet, PDBs, sync-hooks):
![ArgoCD crm-stack](docs/screenshots/argocd-crm-stack.png)

**ArgoCD — prometheus-stack network view** (services wired to pods, 17 resources Healthy):
![ArgoCD prometheus-stack](docs/screenshots/argocd-prometheus-stack.png)

**Grafana — CRM SLO dashboard** (defined as code in [`k8s/manifests/crm-dashboard-configmap.yaml`](k8s/manifests/crm-dashboard-configmap.yaml)) — live traffic through the NLB, including real internet background noise answered with 404s:
![Grafana CRM dashboard](docs/screenshots/grafana-crm-dashboard.png)
